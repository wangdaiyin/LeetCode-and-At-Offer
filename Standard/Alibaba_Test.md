...
根据输入范例我画了一个草图，如下：
![manuscript]:E:\myGithub\LeetCode-and-At-Offer\Standard\Alibaba_Test.jpg
...
*多说几句*：之前我没有接触过二分图匹配也没看到过稳定婚姻问题，开始拿到这道题，想着是组合优化问题，考虑过贪心策略（如在满足约束的情况下，每次取最大匹配数的对，直到所有人都已配对）、动态规划（两个变量[男生数和女生数]间是否可由其子问题计算而来，当时没想清楚递推式，感觉约束复杂也缺少理论上的支撑）、回溯法（类比于在所有满足约束的分组匹配的搜索空间中寻求最大化等，但思虑一番，脑子都是一片浆糊……直到后来看到二分图匹配中的稳定婚姻问题，算是有了一点探索的方向……知识覆盖面的重要性！
